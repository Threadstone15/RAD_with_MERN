...
router.get("/fetchStudentData/:studentID", async (req, res) => {
  console.log(
    `Got a request to fetchStudentData for studentID: ${req.params.studentID}`
  );
  ...
  try {
    ...
    console.log(
      `Fetching student with ID: ${studentObjectId} from the database`
    );

    const student = await Student.findById(studentObjectId).populate(
      "classIds"
    );
...
router.get("/fetchPaymentData/:studentID", async (req, res) => {
  console.log(
    `Got a request to fetchPaymentData for studentID: ${req.params.studentID}`
  );
  ...
  try {
    ...
    console.log(
      `Fetching payments for student with ID: ${studentID} from the database`
    );

    // Assuming studentID is a string
    const payments = await Payment.find({ studentID: studentID }).populate(
      "classID"
    );
